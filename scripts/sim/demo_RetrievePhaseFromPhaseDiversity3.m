...
% [defocus, astigmatism, astigmatism45Â°, comaX, comaY, spherical, trefoil, trefoil45Â°, ...
% secondAstigmatism, secondAstigmatism45Â°, secondComaX, secondComaY, ...],
...
[~, IntensityCenterZ3( :, :, 3), ~, ~, ~, ~, ~] = CalcOAMBeamFTFromAberrations2(FTSize, gridSize, gaussianC, rLimit, ZBaseAberration, dx, dy, order);

% This one has OAM 2, -0.5ast
order = lDiv(4);
ZBaseAberration = kDiv{4} + ZAberration;
[~, IntensityCenterZ3( :, :, 4), ~, ~, ~, ~, ~] = CalcOAMBeamFTFromAberrations2(FTSize, gridSize, gaussianC, rLimit, ZBaseAberration, dx, dy, order);
